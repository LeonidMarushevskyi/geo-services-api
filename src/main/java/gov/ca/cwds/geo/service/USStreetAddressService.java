...
public class USStreetAddressService {
  ...
  public USStreetAddressService(SmartyStreetsDAO smartyStreetsDAO, Client client) {
    this.smartyStreetsDAO = smartyStreetsDAO;
    this.client = client;
  }

  @Loggable
  public ValidatedAddressDTO[] validateSingleUSAddress(String street, String city, String state,
      String zipCode) {
    ...
    return candidates.stream().map(this::toValidatedAddressDTO).toArray(ValidatedAddressDTO[]::new);
  }
  ...
  ValidatedAddressDTO[] validateBatchAddress(Address[] addresses) {
    ...
    return candidates.stream().map(this::toValidatedAddressDTO).collect(Collectors.toSet())
  ...
  private ValidatedAddressDTO toValidatedAddressDTO(Candidate candidate) {
    ...
    return new ValidatedAddressDTO(candidate.getDeliveryLine1(), components.getCityName(),
        components.getState(), null, components.getZipCode(), components.getPlus4Code(),
        metadata.getLongitude(), metadata.getLatitude(), isDeliverableMatchCode(dpvMatchCode));
  }

  private boolean isDeliverableMatchCode(String dpvMatchCode) {
    return "Y".equals(dpvMatchCode) || "S".equals(dpvMatchCode) || "D".equals(dpvMatchCode);
  }
  ...
  public Batch createSmartyStreetsBatch(Address[] addresses) throws BatchFullException {
  ...
  public Lookup createSmartyStreetsLookup(String street, String city, String state, String zipCode) {
    Lookup lookup = new Lookup();
